    ...
    if (includeDefaultModule || includeGlobalModule) {
      ...
      if (includeGlobalModule && !includeDefaultModule) {
        if (!_util.default.isModuleStateValid(store[_constant.MODULE_GLOBAL])) {
          ...
          Object.keys(store[_constant.MODULE_GLOBAL]).forEach(function (key) {
            globalStateKeys.push(key);
            pureGlobalStateKeys.push(key);
          });
          invalidKeyCount += 1;
...
function bindNamespacedKeyReducerToCcContext(namespacedKeyReducer) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    if (!_util.default.verifyActionType(actionType)) {
      throw _util.default.makeError(_constant.ERR.CC_REDUCER_ACTION_TYPE_NAMING_INVALID, " actionType:" + actionType + " is invalid!");
