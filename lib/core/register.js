  ...
  return function (ReactClass) {
    ...
    function (_ReactClass) {
      ...
      function CcClass(props, context) {
        ...
        _util.default.bindThis((0, _assertThisInitialized2.default)((0, _assertThisInitialized2.default)(_this)), ['$$bindDataToCcClassContext', '$$mapCcToInstance', '$$changeStateWithClosure', '$$changeState', '$$broadcastState']);
      ...
      _proto.componentDidMount = function componentDidMount() {
        ...
        if (!isStateEmpty && ccOption.syncState) {
          ...
          this.cc.doReactSetState(_constant.SYNC_FROM_CC_STORE, sharedState);
        }
      ...
      _proto.$$changeState = function $$changeState(state, _temp7) {
        ...
        if (module === currentModule) {
          ...
          this.cc.doReactSetState(_constant.CHANGE_BY_SELF, state, function () {
            if (_this3.cc.ccState.ccOption.syncState) {
              // note!!! if syncState == false, other ccIns will have not change to receive new state
              _this3.cc.doBroadcastState(_constant.BROADCAST_TRIGGERED_BY_CC_INSTANCE_METHOD, module, state, false);
            } else if (forceSync) {
              _this3.cc.doBroadcastState(_constant.BROADCAST_TRIGGERED_BY_CC_INSTANCE_METHOD, module, state, true);
            } else {// other ccIns will have not change to receive new state
            }
          }, reactCallback);
        } else {
          ...
          this.cc.doBroadcastState(_constant.BROADCAST_TRIGGERED_BY_CC_INSTANCE_METHOD, module, state, true);
        }
      }; //{ module, forceSync, cb }
          ...
          ccKeys.forEach(function (ccKey) {
            ...
            if (ccKey !== currentCcKey) {
              ...
              if (ref) {
                ...
                if (option.syncState) {
                  ref.cc.doReactSetState(_constant.SYNC_FROM_CC_INSTANCE, sharedStateForCurrentCcClass);
                }
              }
            }
          });
