...
function register(ccClassKey, _temp) {
  ...
  if (!_ccContext.default.isModuleMode) {
    if (module !== _constant.MODULE_GLOBAL) {
      throw _util.default.makeError(_constant.ERR.CC_REGISTER_A_MODULE_CLASS_IN_NONE_MODULE_MODE, "module:" + module);
    }

    if (_reducerModule != _constant.MODULE_GLOBAL) {
      throw _util.default.makeError(_constant.ERR.CC_REGISTER_A_MODULE_CLASS_IN_NONE_MODULE_MODE, "reducerModule:" + _reducerModule);
    }
  } else {
    if (!_state[module]) {
      throw _util.default.makeError(_constant.ERR.CC_CLASS_STORE_MODULE_INVALID, "module:" + module);
    }

    if (!_reducers[_reducerModule]) {
      throw _util.default.makeError(_constant.ERR.CC_CLASS_REDUCER_MODULE_INVALID, "reducerModule:" + _reducerModule);
    }
  }
  ...
  if (contextMap[ccClassKey] !== undefined) {
    throw _util.default.makeError(_constant.ERR.CC_CLASS_KEY_DUPLICATE, "ccClassKey:" + ccClassKey + " duplicate");
  } else {
    ...
    function (_ReactClass) {
      ...
      _proto.bindDataToCcClassContext = function bindDataToCcClassContext(ccClassKey, ccUniqueKey, ccOption) {
        ...
        if (classContext.ccKeys.includes(ccUniqueKey)) {
          throw _util.default.makeError(_constant.ERR.CC_CLASS_INSTANCE_KEY_DUPLICATE, vbi("ccClass:" + ccClassKey + ",ccKey:" + ccUniqueKey));
        }
        ...
        if (!ccOption) classContext.ccKey_option_[ccUniqueKey] = {
          ...
          if (!_util.default.verifyCcOption(ccOption)) {
            throw _util.default.makeError(_constant.ERR.CC_CLASS_INSTANCE_OPTION_INVALID);
          }
      ...
      _proto.mapCcToInstance = function mapCcToInstance(ccClassKey, ccKey, ccUniqueKey, ccOption, ccClassContext, module, reducerModule, sharedStateKeys, reactSetState) {
        ...
        this.cc = {
          ...
          dispatch: function dispatch(_temp2) {
            ...
            var targetReducerMap = _ccContext.default.reducer._reducers[targetModule];
      ...
      _proto.syncStateToOtherCcComponent = function syncStateToOtherCcComponent(moduleName, state) {
        ...
        var ccClassKeys = _ccContext.default.moduleName_ccClassKeys_[moduleName]; //these ccClass subscribe the same module's state
