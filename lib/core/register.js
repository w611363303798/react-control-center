    ...
    function (_ReactClass) {
      ...
      _proto.componentDidMount = function componentDidMount() {
        ...
        if (ccOption.syncState) {
          ...
          if (!isSharedStateEmpty && isSelfStateEmpty) {
            this.cc.prepareReactSetState(_constant.SYNC_FROM_CC_CLASS_STORE, newSharedState);
          } else if (isSharedStateEmpty && !isSelfStateEmpty) {
            this.cc.prepareReactSetState(_constant.SYNC_FROM_CC_REF_STORE, newSelfState);
          } else if (!isSharedStateEmpty && !isSelfStateEmpty) {
            this.cc.prepareReactSetState(_constant.SYNC_FROM_CC_CLASS_STORE_AND_REF_STORE, (0, _extends2.default)({}, newSharedState, newSelfState));
          }
        ...
        this.cc = {
          ...
          prepareReactSetState: function prepareReactSetState(changeWay, state, next, reactCallback) {
            if (storedStateKeys.length > 0) {
              ...
              if (!isStateEmpty) {
                _ccContext.default.refStore._state[ccUniqueKey] = newState;
              }
            }
              ...
              ccKeys.forEach(function (ccKey) {
                ...
                if (ccKey !== currentCcKey) {
                  ...
                  if (ref) {
                    ...
                    if (option.syncState) {
                      ref.cc.prepareReactSetState(_constant.SYNC_FROM_CC_INSTANCE_STATE, sharedStateForCurrentCcClass);
                    }
                  }
                }
              });
      ...
      _proto.$$changeState = function $$changeState(state, _temp7) {
        ...
        if (module === currentModule) {
          ...
          this.cc.prepareReactSetState(_constant.CHANGE_BY_SELF, state, function () {
            if (_this3.cc.ccState.ccOption.syncState) {
              // note!!! if syncState == false, other ccIns will have not change to receive new state
              _this3.cc.prepareBroadcastState(_constant.BROADCAST_TRIGGERED_BY_CC_INSTANCE_METHOD, module, state, false);
            } else if (forceSync) {
              _this3.cc.prepareBroadcastState(_constant.BROADCAST_TRIGGERED_BY_CC_INSTANCE_METHOD, module, state, true);
          ...
          this.cc.prepareBroadcastState(_constant.BROADCAST_TRIGGERED_BY_CC_INSTANCE_METHOD, module, state, true);
        }
      }; //{ module, forceSync, cb }
