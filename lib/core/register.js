...
function handleCcFnError(err, __innerCb) {
  if (err) {
    if (__innerCb) __innerCb(err);else if (_ccContext.default.errorHandler) _ccContext.default.errorHandler(err);else justWarning(err);
  }
}
/*
options.module = 'xxx'
...
function register(ccClassKey, _temp) {
  ...
  return function (ReactClass) {
    ...
    var TargetClass = extendReactComponent ? _react.default.Component : ReactClass;
