...
function mapCcClassKeyAndCcClassContext(ccClassKey, moduleName, originalSharedStateKeys, originalGlobalStateKeys,
  ...
  helper.buildCcClassContext(ccClassKey, moduleName, originalSharedStateKeys, originalGlobalStateKeys,
    sharedStateKeys, globalStateKeys, stateToPropMapping, isPropStateModuleMode);
}
...
function mapModuleAssociateDataToCcContext(extendInputClass, ccClassKey, stateModule, sharedStateKeys, globalStateKeys, stateToPropMapping, isPropStateModuleMode) {
  ...
  helper.mapModuleAndCcClassKeys(stateModule, ccClassKey);
      ...
      const CcClass = class CcClass extends TargetClass {
        ...
        constructor(props, context) {
          try {
            ...
            const { ccKey: newCcKey, ccUniqueKey, isCcUniqueKeyAutoGenerated } = helper.computeCcUniqueKey(isSingle, ccClassKey, ccKey);
            const ccClassContext = ccClassKey_ccClassContext_[ccClassKey];
            helper.setRef(this, isSingle, ccClassKey, newCcKey, ccUniqueKey, ccOption);
            this.__$$mapCcToInstance(
              isSingle, asyncLifecycleHook, ccClassKey, newCcKey, ccUniqueKey, isCcUniqueKeyAutoGenerated, storedStateKeys,
          ...
          this.cc = {
            ...
            __effect: (targetModule, userLogicFn, extra, lazyMs, ...args) => {
              ...
              return this.cc.__promisifiedInvokeWith(userLogicFn, {
                ...
                calledBy: EFFECT, fnName: userLogicFn.name, lazyMs
              }, ...args);
            },
            ...
            __xeffect: (targetModule, userLogicFn, extra, lazyMs, ...args) => {
              const { ccKey } = extra;
              const dispatch = this.__$$getDispatchHandler(STATE_FOR_ONE_CC_INSTANCE_FIRSTLY, currentModule, currentReducerModule, null, null, -1, ccKey);
              const thisCC = this.cc;
              return thisCC.__promisifiedInvokeWith(userLogicFn, {
                ccKey, stateFor: STATE_FOR_ALL_CC_INSTANCES_OF_ONE_MODULE, dispatch, lazyMs,
            ...
            __invokeWith: (userLogicFn, executionContext, ...args) => {
              const {
                ...
                cb, __innerCb, type, reducerModule, calledBy, fnName, lazyMs = -1
              } = executionContext;
              isStateModuleValid(targetModule, currentModule, cb, (err, newCb) => {
                if (err) return handleCcFnError(err, __innerCb);
                if (context) {
                  const executionContextForUser = {
                    ...executionContext,
                ...
                co.wrap(userLogicFn)(...args).then(partialState => {
                  ...
                  this.$$changeState(partialState, { ccKey, stateFor, module: targetModule, forceSync, cb: newCb, type, reducerModule, changedBy: CHANGE_BY_SELF, calledBy, fnName, lazyMs });
                }).then(() => {
            ...
            dispatch: ({
              ccKey, stateFor, module: inputModule, reducerModule: inputReducerModule,
              ...
              isStateModuleValid(targetStateModule, currentModule, reactCallback, (err, newCb) => {
                ...
                const executionContext = {
                  ...
                  payload, forceSync, cb: newCb, context: true, __innerCb, calledBy: DISPATCH, lazyMs
                };
        ...
        $$changeState(state, {
          ...
          forceSync, cb: reactCallback, type, reducerModule, calledBy, fnName, lazyMs = -1 } = {}
        ) {//executionContext
            ...
            if (module == MODULE_GLOBAL) {
              ...
              if (module === currentModule) {
                ...
                this.cc.prepareReactSetState(changedBy || CHANGE_BY_SELF, state, () => {
                  ...
                  if (forceSync === true) {
                    this.cc.prepareBroadcastState(stateFor, btb, module, state, true, lazyMs);
                  } else if (ccState.ccOption.syncSharedState) {
                    this.cc.prepareBroadcastState(stateFor, btb, module, state, false, lazyMs);
                  } else {
        ...
        __$$getDispatchHandler(stateFor, originalComputedStateModule, originalComputedReducerModule, inputType, inputPayload, lazyMs = -1, ccKey) {
              ...
              this.cc.dispatch(
                {
                  ...
                  cb: _cb, __innerCb: _promiseErrorHandler(resolve, reject), lazyMs: _lazyMs, ccKey
                });
            }).catch(helper.catchCcError);
          }
        }
        ...
        $$domDispatch(event) {
          ...
          const handler = this.__$$getDispatchHandler(STATE_FOR_ONE_CC_INSTANCE_FIRSTLY, module, reducerModule, type, payload, cclazyms, ccKey);
          handler();
        }
